...
class FlatRateESG:
    ...
    def generate(self, num_paths, num_years):
        base_rate_matrix = np.full((num_paths, num_years), self.base_rate) # Generates a matrix of size paths and years, with all the values being the base_rate
        ...
        noise = np.random.normal(loc=0.0, scale=0.005, size=(num_paths, num_years))
        return base_rate_matrix + noise
